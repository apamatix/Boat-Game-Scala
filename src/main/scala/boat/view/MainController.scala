...
import boat.MainApp
import boat.model.{Blank, Current, GameLogic, Item, Player, River, Trap}
...
class MainController(
                      ...
                      private val secondaryBackground: ImageView, private val primaryBackground: ImageView, private val messageText: Text, private val backButton: Button, private val item0: Circle, private val item1: Circle, private val item2: Circle,
  ...
  secondaryBackground.image = new Image("/image/playerBackground.jpg", 218, 555, false, false)
  primaryBackground.image = new Image("/image/riverBackground.jpg", 684, 555, false, false)
  ...
  if (difficulty == 1) game.river.createRiver(1)
  else game.river.createRiver(2)
  ...
  for (item <- game.river.rivers) {
    var index: Int = game.river.rivers.indexOf(item)
  ...
  def newRound(player1Location: Int, player2Location: Int): Unit = {
    if (game.player1.location != 99 && game.player2.location != 99) {
      ...
      messageText.setText("Player 1, Roll the dice")

      // roll dice when player 1 clicks roll dice button
      player1Dice.onAction = (event: ActionEvent) => {
        diceNum = game.player1.diceRoll()

        // disable player 1 roll dice button
        player1Dice.setDisable(true)

        // display number rolled
        messageText.setText("Player 1 has rolled a " + diceNum + ", Move forward " + diceNum + " step(s)")
      ...
      player2Dice.onAction = (event: ActionEvent) => {
        diceNum = game.player2.diceRoll()

        // disable player 2 roll dice button
        player2Dice.setDisable(true)

        // display number rolled
        messageText.setText("Player 2 has rolled a " + diceNum + ", Move forward " + diceNum + " step(s)")
  ...
  def enableMoveButtons(playerLocation: Int, desiredLocation: Int, playerNum: Int): Unit = {
    ...
    if (playerLocation != desiredLocation) {
      ...
      if (playerNum == 1) { // player 1 won
        messageText.setText("Game over. Congratulations, Player 1 has won!") // display player 1 won message

        // disable move buttons
        player1Left.setDisable(true)
        player1Right.setDisable(true)
      } else { // player 2 won
        messageText.setText("Game over. Congratulations, Player 2 has won!") // display player 2 won message
  ...
  def checkPlayerLocation(playerLocation: Int, playerNum: Int): Unit = {
    if (!(game.river.rivers(playerLocation).isInstanceOf[Blank])) {
      if (playerNum == 1) {
        desiredLocation = encounterItem(game.river.rivers(playerLocation), playerLocation, 1)
        ...
        desiredLocation = encounterItem(game.river.rivers(playerLocation), playerLocation, 2)
      ...
      if (playerNum == 1 && game.player1.location != 99) { // switch to player 2
        ...
        messageText.setText("Player 2, please roll the dice")
  ...
  // function to randomly generate strength of current or trap
  def encounterItem(item: Item, location: Int, playerNum: Int): Int = {
    if (item.isInstanceOf[Current]) {
      (item.asInstanceOf[Current]).createStrength()

      // display strength of current
      if (playerNum == 1) {
        messageText.setText("Player 1 has encountered a current of strength " + item.strength + ", move forward " + item.strength + " step(s)")
      } else {
        messageText.setText("Player 2 has encountered a current of strength " + item.strength + ", move forward " + item.strength + " step(s)")
      }

      // set desired locaiton
      desiredLocation = location + item.strength
      if (desiredLocation > 99) {
        desiredLocation = 99
      }
      ...
      (item.asInstanceOf[Trap]).createStrength()

      // display strength of trap
      if (playerNum == 1) {
        messageText.setText("Player 1 has encountered a trap of strength " + item.strength + ", move backward " + item.strength + " step(s)")
      } else {
        messageText.setText("Player 2 has encountered a trap of strength " + item.strength + ", move backward " + item.strength + " step(s)")
      }
